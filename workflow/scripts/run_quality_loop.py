...
def main():
 ...
 subprocess.run([sys.executable,str(TOOLS/'evaluate_document.py'),str(args.document),'--out',str(review)],check=True)
 ...
 defects=list(report.get('missing_sections',[]))+list(report.get('ai_style_risks',{}).keys())
 ...
 print(json.dumps({'review':str(review),'verdict':report['verdict'],'coverage_score':report['coverage_score'],'defects':defects},ensure_ascii=False,indent=2)); return 0
